...
class AddTaskActivity : AppCompatActivity(), DatePickerFragment.DialogDateListener {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            R.id.action_save -> {
                //TODO 12 : Create AddTaskViewModel and insert new task to database - FINISHED
                binding.apply {
                    val title = addEdTitle.text.toString()
                    val description = addEdDescription.text.toString()
                    val date = dueDateMillis

                    val newTask = Task(0, title, description, date, false)
                    addTaskViewModel.insertTask(newTask)
                    Log.d("AddTaskActivity", "Menambahkan : $newTask")
                    finish()
                    Toast.makeText(applicationContext, "New task successfully added", Toast.LENGTH_SHORT).show()
                }
                true
            }
